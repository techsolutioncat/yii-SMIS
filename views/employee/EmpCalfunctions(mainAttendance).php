...
<div id="calender_section">
		...
		<table class="table table-striped table-bordered" cellspacing="0" width="100%" style="margin-left: -250px;">
		...
		<th></th>
			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						//echo $currentDate;
			  $attendance =AttendanceMain::find()->where(['fk_user_id'=>$q->user_id,'checktype'=>'I']) 
		      ->andWhere(['between', 'datetime', $currentDate.' 00:00:00', $currentDate.' 23:59:59'])->orderBy(['id' => SORT_DESC])->limit(1)->all();
			//	echo $q->user_id;
               // $attendance =AttendanceMain::find()->where(['fk_user_id'=>$q->user_id,'checktype'=>'I'])->All();	
            ...
            $getTimeSettings=Settings::find()->where(['fk_branch_id'=>Yii::$app->common->getBranch()])->one();
					  ...
					  // echo '<span style="background: black;">asdf</span>';
						...
						<?php 
							...
							//echo $currentDate;
								 ...
								 $date= date('d',strtotime($attend->datetime));
								 if($getCurrentD == $date){
								 	
										 $newd=$attend->datetime;
                                           
                                            $getSetingTime=date("H:i",strtotime($getTimeSettings->school_time_in));
										    $latetime=date('H:i',strtotime($newd));
                                            if($latetime < $getSetingTime){
                                            echo '<span id="emp_'.$q->emp_id.date('d').'" class="policies at_sp at_leave_present">P</span>';
                                            }else if($latetime > $getSetingTime){
                                            echo '<span id="emp_'.$q->emp_id.date('d').'" class="leaveid at_sp at_leave_com">LC</span>';
                                            }
                                         //echo '<span id="emp_'.$q->emp_id.date('d').'" style="color:green">'.date('H:i',strtotime($attend->datetime)).'</span>';
